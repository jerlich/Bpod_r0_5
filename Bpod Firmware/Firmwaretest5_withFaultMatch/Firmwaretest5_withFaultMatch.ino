    ...
    case 'I': // Read and return digital input line states
        ...
        switch (LowByte) {
          ...
          case 'P': // Read port digital input line
            ThirdByte = analoglogic(PortAnalogInputLines[SecondByte]);
          break;
...
int faultMatch(int x){
  int flag;

if ((analogRead(PortAnalogInputLines[x]))>1000)
{flag=1;
}

 if((800>(analogRead(PortAnalogInputLines[x])))&&((analogRead(PortAnalogInputLines[x]))>650))
{flag=2;
}

 if((500>(analogRead(PortAnalogInputLines[x])))&&((analogRead(PortAnalogInputLines[x]))>400))
{flag=3;
}
 
 if((390>(analogRead(PortAnalogInputLines[x]))))
{flag=4;
}
