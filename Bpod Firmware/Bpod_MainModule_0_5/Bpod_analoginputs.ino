    ...
    case 'R':  // Run State Matrix
      ...
      for (int x = 0; x < 8; x++) {
        if (PortInputsEnabled[x] == 1) { 
          PortInputLineValue[x] = analoglogic(); // Read each photogate's current state into an array
      ...
      while (MatrixFinished == false) {
        ...
        if (SerialUSB.available() > 0) {
         ...
         if (OverrideFlag == false) {
           ...
           for (int x = 0; x < 8; x++) {
             if (PortInputsEnabled[x] == 1) { 
              PortInputLineValue[x] = analoglogic();
             }
          }
          for (int x = 0; x < 2; x++) {
            BNCInputLineValue[x] = digitalRead(BncInputLines[x]);
...
boolean analoglogic()
 {int x;
   if(analogRead(PortAnalogInputLines[x])<400) {return LOW;}
   else{return HIGH;}
